classdef zernikeStats
    ...
    methods (Static)
        ...
        function [ARn, PSDs] = computeARmodel(atm,tel,zern,FRange,ModelFitOrd, NptsFit, zpsdFlag,do_plots)
            %%
            %{
            ------------HEADER-----------------
            ...
            INPUT VARS
             atm, tel,zern     :: atmosphere, telescope and zernike objects
             FRange            :: The temporal vector of points were the correlation is evaluated
             Npts              :: Number of points to consider in the fitting
             ModelFitOrd       :: Order of the model to fit
             zpsdFlag          :: Use PSD-based method or spatio-angular
                                  methood
             do_plots          :: flag to plot results
            OUTPUT VARS
             ARn               :: [struct] A, B, Sigma_n -- AR-n coefficients (mode-by-mode)
             PSDs              :: [struct] The temporal PSD of modes and
            ...
            Fmax   = max(FRange);
            Nfreq  = 5000;
            flin     = linspace(0,Fmax,Nfreq+1)';
            dnu    = mean(diff(flin));
            Ts     = tel.samplingTime;
            A      = zeros(1, Nmodes*atm.nLayer);
            if ModelFitOrd == 2
                B  = zeros(1, Nmodes*atm.nLayer);
            end
            if ModelFitOrd == 3
                C  = zeros(1, Nmodes*atm.nLayer);
            end
            Sigma_n= zeros(1, Nmodes*atm.nLayer);
            Tfit   = NptsFit*Ts; 
            if ~exist('zpsdFlag','var')
                zpsdFlag = 1;
            end
            if zpsdFlag
                nu = logspace(-5,3,500)*zpsdFlag;
                if ~exist('Zpsd_54modes.mat','file')
                    
                    fprintf('Computing zernike auto-correlation functions using temporal PSDs\n [fmin:fmax]=%d:%dHz\n',nu(1), nu(end))
                    ZPSD = zernikeStats.multipleSpectra(nu,atm,zern,tel);
                    
                    save Zpsd_54modes ZPSD
                else
                    load Zpsd_54modes
                ...
                % Compute spatio-angular correlation functions based on spatio-angular covariance matrices
                NptsCov = NptsFit;
                SpAngPhaseCovMat = cell(NptsCov,Nlayers);
                CphiAngCov = zeros(zern.nMode,NptsCov,Nlayers);
                for kLayer = 1:atm.nLayer
                    ...
                    myatm = atmosphere(.5e-5,atm.r0,atm.L0,...
                        'altitude',altitude,...
                        'fractionnalR0',fractionalR0,...
                        'windSpeed',windSpeed,...
                        'windDirection',windDirection);
                    
                    dtheta = tel.samplingTime*windSpeed(2)/altitude(2);
                    parfor kr = 1:NptsCov
                        alpha = (kr-1)*dtheta;
                        alpha = 1/2*alpha*180/pi*3600;
                        ast = source('asterism',{[2,alpha*cougarConstants.arcsec2radian,0]});
                        SpAngPhaseCovMat{kr,kLayer} = phaseStats.zernikeAngularCovariance(zern,myatm,ast);
                        %CphiAngCov(:,kr,kLayer) = diag(S{kr,kLayer}./S{1,kLayer});
                    end
                end
                % normalize results (outside the initial loop to allow parfor)
                for kr = 1:NptsCov
                    for kLayer = 1:atm.nLayer
                        CphiAngCov(:,kr,kLayer) = diag(SpAngPhaseCovMat{kr,kLayer}./SpAngPhaseCovMat{1,kLayer});
                    end
                end
            end
            ...
            %----------------------------------------------------------------------
            f    = flin(1:end)'; %may or may not be needed
            Fmax = f(end);
            dt   = 1/(2*Fmax);
            tmax = 1/dnu;
            ...
            for kLayer = 1:atm.nLayer
                for kmode=1:Nmodes
                    ...
                    if zpsdFlag
                        ...
                        PSDi_interp = interp1(nu, PSDi, flin);
                        sp          = PSDi_interp(2:end)'; % interpolation at zero is not to be taken into account
                        corr        = utilities.correl_fcn_WienerKhinchin(sp);
                        correl      = corr(1,end/2+1:end);
                    else
                        correl = CphiAngCov(kmode,:,kLayer);
                        tvec   = 0:Ts:(Nfreq-1)*Ts;
                        dt     = Ts;
                        %f = flin(2:end)';
                        %dnu = mean(diff(f));
                        %tmax     = 1/dnu;
                    ...
                    coeff = utilities.bestfitmodel(correl,tvec, floor(Tfit/dt), ModelFitOrd,0.1,80,1);

                    
                    if length(ModelFitOrd) ~= 1
                        p1Ord1 = coeff(1);
                        syswindOrd1 = tf(p1Ord1,[1 p1Ord1]);
                        
                        p1Ord2 = coeff(2);
                        p2 = coeff(3);
                        syswindOrd2 = tf(p1Ord2*p2,[1 p1Ord2+p2 p1Ord2*p2]);
                        
                        [mag1]   = bode(syswindOrd1,2*pi*f);
                        mag1     = reshape(mag1, 1,length(mag1));
                        sp_fullOrd1 = [0 mag1(length(mag1):-1:2)  mag1];
                        Cphi1    = fftshift(fft(fftshift(sp_fullOrd1.^2)));     % autocorrelation thru Wiener-Khinchine theorem
                        Cphi1    = real(Cphi1);
                        Cphi1    = Cphi1/max(Cphi1);
                        Np1      = length(Cphi1);
                        
                        [mag2]   = bode(syswindOrd2,2*pi*f);
                        mag2     = reshape(mag2, 1,length(mag2));
                        sp_fullOrd2 = [0 mag2(length(mag2):-1:2)  mag2];
                        Cphi2    = fftshift(fft(fftshift(sp_fullOrd2.^2)));     % autocorrelation thru Wiener-Khinchine theorem
                        Cphi2    = real(Cphi2);
                        Cphi2    = Cphi2/max(Cphi2);
                        Np2      = length(Cphi2);
                        
                        
                    elseif ModelFitOrd == 1
                        p1      = coeff(1);
                        syswind = tf(p1,[1 p1]);
                    ...
                    if do_plots
                        figure(kmode+100), hold on
                        %plot(tvec(1:NptsFit), CphiAngCov(kmode,:,kLayer),'r--')
                        %axis([0 .1, 0.5 1.2])
                        
                        plot(tvec, corr(floor(Np/2+1):floor(Np/2+1)+length(tvec)-1),'b.');
                    end
                    %----------------------------------------------------------------------
                    ...
                    dtz      = 1/(2*fMaxAR);
                    z        = tf('z',Ts);
                    fz       = linspace(0,fMaxAR,Nfreq*(fMaxAR/Fmax));
                    tz       = linspace(-tmax/2,tmax/2-dtz, tmax/dtz);
                    tz       = tz(1,end/2+1:end);
                    
                    if length(ModelFitOrd) ~= 1
                        a1Ord1 = -exp(-Ts*p1Ord1);
                        H1 = Ts/(1-a1Ord1/z);
                        [mag1,~,~]= bode(H1,2*pi*fz);
                        ...
                        A1(kmode+Nmodes*(kLayer-1)) = -a1Ord1;
                        A{1} = A1;
                        a1Ord2       = -2*exp(-Ts*1/2*(p1Ord2+p2))*cos(sqrt(p1Ord2*p2)*Ts*sqrt(1-1/4*(p1Ord2^2+2*p1Ord2*p2+p2^2)/(p1Ord2*p2)));
                        b1       = exp(-(p1Ord2+p2)*Ts);
                        H2        = (p1Ord2*p2*Ts)/(1 + a1Ord2/z + b1/(z^2));
                        [mag2,~,~]= bode(H2,2*pi*fz);
                        ...
                        A2(kmode+Nmodes*(kLayer-1)) = -a1Ord2;
                        B(kmode+Nmodes*(kLayer-1)) = -b1;
                        Sigma_n(kmode+Nmodes*(kLayer-1)) = -a1Ord2/(1+b1)*trapz(nu, PSDi);
                        A{2} = A2;
                    elseif ModelFitOrd == 1
                        a1       = -exp(-Ts*p1);
                        H        = Ts/(1-a1/z);
                    ...
                    elseif ModelFitOrd == 2
                        ...
                        A(kmode+Nmodes*(kLayer-1)) = -a1;
                        B(kmode+Nmodes*(kLayer-1)) = -b1;
                        %Sigma_n(kmode+Nmodes*(kLayer-1)) = -a1/(1+b1)*trapz(nu, PSDi);
                    else
                        z1 = exp(-p1*Ts);
                        z2 = exp(-p2*Ts);
                        z3 = exp(-p3*Ts);
                        Hz = zpk([], [z1 z2 z3], 1,Ts);
                        Hz = tf(Hz); % convert to tf
                        pp = get(Hz,'den');
                        pp = pp{1};
                        A(kmode+Nmodes*(kLayer-1)) = -pp(2);
                        B(kmode+Nmodes*(kLayer-1)) = -pp(3);
                        C(kmode+Nmodes*(kLayer-1)) = -pp(4);
                    end
                    if do_plots
                        plot(tz, corr(floor(Np/2+1):end),'go');
        ...
        function out = multipleSpectra(nu,atm,zern,tel)
            ...
            parfor kmode=1:nMode
                out(kmode,:,:)  = zernikeStats.temporalSpectrum(nu,atm,tel,kmode);
            end
