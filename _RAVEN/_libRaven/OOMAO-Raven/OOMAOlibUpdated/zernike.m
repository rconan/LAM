...
%                 end
        ...
        function relay(obj,src)
            ...
            for kSrc=1:nSrc
                ...
                switch nC
                    case 1
                        src(kSrc).phase     = utilities.toggleFrame(obj.p_p*obj.c,3);                        
  %                      src(kSrc).phase     = utilities.toggleFrame(obj.p_p*obj.c*src(kSrc).waveNumber,3);
                    case nSrc
                        src(kSrc).phase     = utilities.toggleFrame(obj.p_p*obj.c(:,kSrc),3);
...
%                        src(kSrc).phase     = utilities.toggleFrame(obj.p_p*obj.c*src(kSrc).waveNumber,3);
        ...
        function out = fourier(obj,f,o)
            ...
            function out1 = fun(zj,n,m)
                ...
                p = pi.*krkr.*((-1).^zj-1)/4;
                if m ~=0
                    g = (-1)^((n-m)/2).*1i.^(m).*sqrt(2);
                else
                    g = (-1)^(n/2);
                end
                out1 = 4.*sqrt(obj.n+1).*utilities.sombrero(n+1,pi.*f.*obj.D).*...
                    g.*cos(m.*o+p); % factor 4=2*2 from partial integration 0to\infty; other factor 2 to account for + and - temporal frequencies
            end
        end
