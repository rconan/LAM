...
%%% Noisy case:
    ...
    params.noise.framePixelThreshold = 0.2;
...
params.TomographyType = 'LGS'; %% display the diagnostic plots during the simulation.
% params.TomographyType = 'NGS'; %% display the diagnostic plots during the simulation.
...
dAsterism                         = 0.1; % 0.5, 0.75, 1.0, 1.5, 2 diameter in arcmin
...
switch params.TomographyType

    case 'LGS'  %% Laser Tomography
        
            ast =  source('asterism',{[3,arcmin(dAsterism/2),0]}, 'height', 90e3, ...
...
Dzcc = wfs.slopes; %% Matrix  \Gamma of gradients that the WFS can sense
clear ngs
...
%     [Projection_alpha, Palphacell] = AnalyticalSmallFootprintExpansion(zernProj,tel,ast,atm);
...
%%    Closed-loop controller
     dm.coefs(:,1) = dm.coefs(:,1) - gain_cl * Z2U*Rmv*StackSlopeLarge(:);  %%% Tomography
