...
nLenslet = 10;
pixSize = 0.6; %arcsecs
lensletFov = 6; %arcsecs
nPx_lenslet = lensletFov/pixSize;
nPx = nLenslet*nPx_lenslet;
tel = telescope(8,...
...
% % Phase variance to micron rms converter 
 rmsMicron = @(x) 1e6*sqrt(x).*ss(1).wavelength/2/pi;
% figure(13)
...
%% Guide Stars
% gs = source('asterism',{[3,0.75*cougarConstants.arcmin2radian,0]},...
%     'wavelength',photometry.R,'magnitude',0);
% nGs = length(gs);
% %% Science stars ( on-axis oan in GS#1 direction)
% ss = source('zenith',0*cougarConstants.arcsec2radian,...
%     'wavelength',photometry.R);
% nSs = length(ss);
gs = source('asterism',{[3,arcmin(0.75),0]},'wavelength',ngs.wavelength);
nGs = length(gs);
%% Science stars ( on-axis oan in GS#1 direction)
ss = source('zenith',[0,gs(1).zenith],'azimuth',[0,gs(1).azimuth],'wavelength',photometry.H);
nSs = length(ss);
%% WFS noise
...
imgr1 = imager(tel);
imgr1.exposureTime = exposureTime;
imgr1.eeWidth = ircsSlitWidth; % arcsec
imgr1.imgLens.nyquistSampling = 2;

imgr2 = imager(tel);
imgr2.exposureTime = exposureTime;
imgr2.eeWidth = ircsSlitWidth; % arcsec
imgr2.imgLens.nyquistSampling = 2;

% Get a reference frame
for i = 1:nIteration
    ss(1) = ss(1).*tel*dm*imgr1;
    ss(2) = ss(2).*tel*dm*imgr2;
end

imgr1.referenceFrame = imgr1.frame;
imgr2.referenceFrame = imgr2.frame;
%%
