...
%                 psCov     = psCov*atm.la
        ...
        function varargout = spatioAngularCovarianceMatrix(sampling,range,atm,srcAC,varargin)
            ...
            function C = crossCorrelation(srcCC,xAC,yAC,xCC,yCC,...
                ...
                for k=1:nSs*nGs   
                    ...
                    for kLayer=1:nLayer
                        ...
                        % CC added transpose at the end of line
                        %fprintf(' -->> Transposed !\n')
                        %rho   = abs(bsxfun(@minus,zSs,iZ.'))';
                        out   = cstL0CC;
                        index = rho~=0;
            ...
            function S = autoCorrelation(x,y,mask,...
                    ...
                    kGs = reshape( triu( reshape(1:nGs^2,nGs,nGs)) , 1, []);
                    kGs(1) = 1;
                    kGs(kGs==0) = [];
                    ...
                    for k=1:length(kGs)
                        ...
                        for kLayer=1:nLayer
                            ...
                            % CC added treanpose at end of line 
                            %out = cell2mat( myToeplitz(cc,rr) )';
                            out(~mask,:) = [];
                            out(:,~mask) = [];
...
%                     tripleBessel2 = besselj(abs(mi-mj),red);
                    besselsRadialOrder = besselj('J',ni+1,red1).*besselj('J',nj+1,red2);
                    tripleBessel1 = besselj('J',mi+mj,red);
                    tripleBessel2 = besselj('J',abs(mi-mj),red);
            ...
            function out = integrandNgs(x,zi,ni,mi,zj,nj,mj)
                ...
                red1 = a1l.*x;
                red2 = a2l.*x;
                %besselsRadialOrder = besselmx('J',ni+1,red1).*besselj('J',nj+1,red2);
                besselsRadialOrder = besselj(ni+1,red1).*besselj(nj+1,red2);
                f = 0.5*x/pi;
                phasePSD = ...
