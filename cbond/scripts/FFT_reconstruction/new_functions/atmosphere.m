...
%             end
        ...
        function out = fourierPhaseScreen(atm,D,nPixel,nLenslet,nMap)
            ...
            out = zeros(nPixel,nPixel,3,nMap);
            for kMap=1:nMap
                map = psdRoot.*fft2(randn(atm.rngStream,N))./N; % White noise filtering
                map = real(ifft2(map).*fourierSampling).*N.^2;
                % CB edit 29/04/2015
                %----------------------------------------------------------
                mapLO = psdRoot_LO.*fft2(randn(atm.rngStream,N))./N; % White noise filtering
                mapLO = real(ifft2(mapLO).*fourierSampling).*N.^2;
                mapHO = psdRoot_HO.*fft2(randn(atm.rngStream,N))./N; % White noise filtering
                mapHO = real(ifft2(mapHO).*fourierSampling).*N.^2;
                out(:,:,1,kMap) = map(u,u);
                out(:,:,2,kMap) = mapLO(u,u);
                out(:,:,3,kMap) = mapHO(u,u);
                %----------------------------------------------------------
            end
        end
