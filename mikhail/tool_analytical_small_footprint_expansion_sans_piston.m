%> @file tool_analytical_small_footprint_expansion_sans_piston.m.m
%> @brief Computes the Zernike temporal correlation functions using analytical angular covariance routines. This function computes the small footprint expansion @b wihtout @b piston!
...
% ======================================================================
function [Proj Projcell] = tool_analytical_small_footprint_expansion_sans_piston(zernModeMaxProj,tel,gs,atm)
...
Projcell = cell(nGs, atm.nLayer);  %%% Cell array of size NumberOfNGS x number of atmospheric layers.
...
for kLayer = 1:atm.nLayer  %% iterating for each atmosphere layer atm.nLayer;
    for kGuidestar = 1:nGs %% iterating through Guide Stars
        ...
        Projcell{kGuidestar, kLayer} = Proj(:,resort_indices); %% Store the projected coefficients into Cell array
